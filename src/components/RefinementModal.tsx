...
export const RefinementModal: React.FC<RefinementModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#140e1f]/50 backdrop-blur-xs animate-fadeIn font-sans">
      <div className="bg-white border border-[#d8dce6] max-w-3xl w-full h-[85vh] flex flex-col rounded-3xl shadow-2xl overflow-hidden">
        {/* Modal Header */}
        <div className="bg-[#f8f9fc] px-5 py-4 border-b border-[#e2e5ec] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-sm sm:text-base font-bold text-[#2e2640]" style={{ fontFamily: 'var(--font-heading)' }}>
                  Refinamento de Escopo & Apoio ao Coordenador
                </h3>
                <span className="text-[10px] bg-[#edeef4] text-[#364f99] px-2 py-0.5 rounded font-mono border border-[#d8dce6]">
                  {initiative.matchedModule}
                </span>
              </div>
              <p className="text-xs text-[#555065] truncate max-w-md">
                Iniciativa: <span className="text-[#2e2640] font-medium">{initiative.title}</span>
              </p>
            </div>
          </div>
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg bg-white hover:bg-[#edeef4] text-[#555065] hover:text-[#2e2640] border border-[#d8dce6] transition"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="bg-[#f4f5fa] p-3 border-b border-[#e2e5ec] overflow-x-auto flex gap-2">
          {quickPrompts.map((qp, i) => (
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-xs font-medium bg-white hover:bg-[#ff4545] hover:text-white text-[#2e2640] border border-[#d8dce6] transition whitespace-nowrap shrink-0 shadow-2xs"
            >
              {qp.label}
            </button>
          ))}
        ...
        <div className="flex-1 p-5 overflow-y-auto space-y-4 bg-white">
          {messages.map((msg, idx) => (
            ...
            >
              {msg.sender === 'assistant' && (
                <div className="w-7 h-7 rounded-lg bg-[#ff4545]/10 text-[#ff4545] flex items-center justify-center shrink-0 border border-[#ff4545]/25 mt-1">
              ...
              <div
                className={`max-w-[85%] rounded-2xl p-4 text-xs sm:text-sm leading-relaxed relative group ${
                  msg.sender === 'user'
                    ...
                    : 'bg-[#f8f9fc] border border-[#e2e5ec] text-[#2e2640] rounded-bl-none shadow-xs'
                }`}
              >
                ...
                {msg.sender === 'assistant' && (
                  <button
                    ...
                    className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 p-1 rounded bg-white border border-[#d8dce6] text-[#555065] hover:text-[#2e2640] transition"
                    title="Copiar texto"
                  >
                    {copiedIdx === idx ? <Check className="w-3.5 h-3.5 text-[#066d73]" /> : <Copy className="w-3.5 h-3.5" />}
                  </button>
                )}
              </div>
          ...
          {loading && (
            <div className="flex gap-3 items-center text-xs text-[#364f99]">
              <div className="w-7 h-7 rounded-lg bg-[#ff4545]/10 text-[#ff4545] flex items-center justify-center shrink-0 border border-[#ff4545]/25">
        ...
        {/* Input Bar */}
        <div className="p-4 bg-[#f8f9fc] border-t border-[#e2e5ec] flex gap-2">
          <input
            type="text"
            value={question}
            ...
            className="flex-1 bg-white border border-[#d8dce6] rounded-xl px-4 py-2.5 text-xs sm:text-sm text-[#2e2640] placeholder-[#9ba0ab] focus:outline-none focus:ring-1 focus:ring-[#ff4545] focus:border-[#ff4545]"
          />
          ...
          <button
            ...
            onClick={() => handleSend()}
            className="px-5 py-2.5 rounded-xl bg-[#ff4545] hover:bg-[#e03232] disabled:bg-[#edeef4] disabled:text-[#9ba0ab] text-white font-semibold text-xs flex items-center gap-1.5 transition shadow-sm"
          >
